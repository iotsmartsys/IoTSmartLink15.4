...
| Registry de devices pareados do coordenador | `docs/specs/ISSP-Coordinator-Paired-Device-Registry.md` | Proposed v0.4 | Funcional In Progress; migração de validação In Progress; Not Ready; Implementable | `coordinator_154/main/device_registry.{h,c}` + `device_registry_nvs.c`, integrado em `main.c` | Revisão confirma builds ESP32-C6/C3, mas runner exige 13 para uma fonte com 17 casos; precedência pending/unavailable ainda incorreta; G1, G3-N, G5 e critérios obrigatórios seguem pendentes; sem aceite |

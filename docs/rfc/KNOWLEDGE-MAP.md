...
| Registry de devices pareados do coordenador | `docs/specs/ISSP-Coordinator-Paired-Device-Registry.md` | Proposed | In Progress; Not Ready; Needs Clarification | `coordinator_154/main/device_registry.{h,c}` + `device_registry_nvs.c`, integrado em `main.c` | Análise independente v0.2 (seção 16.8, 01/08/2026) confirma `Needs Clarification`: (1) seção 6 deixa checksum/marcador opcional enquanto seção 10/AC-007 exigem reprovar integridade inválida; (2) AC-002/G1+G2 não atravessam falha de `nvs_commit()` no adaptador de produção. Escopo de 13 requisitos/8 AC preservado; baseline `In Progress` não aprovado |

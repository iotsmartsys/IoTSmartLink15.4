...
| Registry de devices pareados do coordenador | `docs/specs/ISSP-Coordinator-Paired-Device-Registry.md` | Proposed v0.4 | Funcional In Progress; migração de validação In Progress; Not Ready; Pending Review | `coordinator_154/main/device_registry.{h,c}` + `device_registry_nvs.c`, integrado em `main.c` | Runner físico ESP32-C3 escrito para os 13 casos existentes, ainda não executado; G1, G3-N, hardware e critérios obrigatórios seguem pendentes; sem aceite |

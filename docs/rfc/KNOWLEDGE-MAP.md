...
| Variantes de firmware por `menuconfig` | `docs/specs/Firmware-Variants-Menuconfig.md` | Proposed; direção integral `Implementable`; B1 a B4 da Fase 2 resolvidos e confrontados, sem bloqueador remanescente | In Progress; Fase 1 concluída e Fase 2 especificada | Fase 1 em `client_154/main/`; Fase 2 define `Door sensor`, segundo board H2, entrada digital, registro unificado e serialização interna dos pending reports | Evidências da Fase 1 preservadas; confronto focado registrado com prontidão recomendada e esclarecimentos C1 a C3; resoluções ainda sem implementação ou evidência |

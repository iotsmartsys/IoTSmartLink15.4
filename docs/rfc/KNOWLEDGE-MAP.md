...
| Registry de devices pareados do coordenador | `docs/specs/ISSP-Coordinator-Paired-Device-Registry.md` | Proposed v0.4 | Funcional In Progress; migração de validação In Progress; Not Ready; Implementable | `coordinator_154/main/device_registry.{h,c}` + `device_registry_nvs.c`, integrado em `main.c` | Runner físico ESP32-C3 escrito para 17 casos (13 anteriores + 4 novos de AC-007), ainda não executado nesta sessão (sem placa; QEMU proibido); build limpo ESP32-C6/ESP32-C3 com ESP-IDF 6.0.1; precedência de `RegistryUnavailable` no comando do host corrigida e rótulos de teste parciais corrigidos; G1, G3-N, G5 e critérios obrigatórios seguem pendentes; sem aceite |

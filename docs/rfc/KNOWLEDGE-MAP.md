...
| Registry de devices pareados do coordenador | `docs/specs/ISSP-Coordinator-Paired-Device-Registry.md` | Proposed | In Progress; Not Ready; Implementable | `coordinator_154/main/device_registry.{h,c}` + `device_registry_nvs.c`, integrado em `main.c` | Revisão de 01/08/2026: build ESP32-C6 aprovado sem warnings; app ESP32-C3 compilado; QEMU 10/10. Aceite bloqueado: fail-closed permite `DATA` com registry indisponível/janela aberta; boot ainda pode apagar toda a NVS; gates AC-002/005/006/007 incompletos; AC-001/008 sem hardware real |

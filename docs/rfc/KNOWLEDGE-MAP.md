...
| Registry de devices pareados do coordenador | `docs/specs/ISSP-Coordinator-Paired-Device-Registry.md` | Proposed | In Progress; Not Ready; Implementable | `coordinator_154/main/device_registry.{h,c}` + `device_registry_nvs.c`, integrado em `main.c` | COORD-REG-001 a 013 implementados; testes Unity/QEMU escritos em `coordinator_154/test_apps/device_registry_test` cobrindo AC-002/003/004/006 e os estados de carga de AC-001/AC-007 com substituto de NVS; build ESP-IDF e execução QEMU não realizados neste ambiente (sem toolchain ESP-IDF); AC-001/AC-008 (hardware real) e a parte de AC-007 com NVS real/sentinela pendentes |

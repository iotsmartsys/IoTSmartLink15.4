...
| Variantes de firmware por `menuconfig` | `docs/specs/Firmware-Variants-Menuconfig.md` | Proposed; revisão de implementabilidade `Implementable` | Não iniciada | Proposta para `client_154/main/{Kconfig.projbuild,firmwares/,boards/}`; nenhum código funcional criado | Ramo proposto do client neste mapa; fluxo, fronteiras, critérios BDD e estratégia do experimento na especificação; confronto do Engenheiro Analista contra `main.cpp`, `CMakeLists.txt` e `SmartSysApp.h` registrado na própria especificação |

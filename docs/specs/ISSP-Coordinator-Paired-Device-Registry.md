...
**Estado do workflow:** Implementação; validação funcional pendente
**Infraestrutura de validação:** target ESP32-C6 corrigido; 24 casos `Not Executed`
**Versão:** 0.4
...
- `docs/rfc/EKOM-CHANGELOG.md`: transação legada `EKM-CHG-0008`;
...
## 16. Estado vigente e relatórios relacionados

O contrato funcional permanece `Proposed` e a implementação `In Progress` até
existir validação suficiente. A correção de target retirou a regressão da
infraestrutura: o runner está vinculado ao ESP32-C6, mas os 24 casos continuam
`Not Executed` por TESTEXEC-009.

- análise: `docs/reports/coordinator-device-registry/analysis/legacy-cycles.md`;
- implementação: `docs/reports/coordinator-device-registry/implementation/legacy-cycles.md`;
- revisão: `docs/reports/coordinator-device-registry/review/legacy-cycles.md`;
- decisões e validação:
  `docs/reports/coordinator-device-registry/validation/architectural-cycles.md`.

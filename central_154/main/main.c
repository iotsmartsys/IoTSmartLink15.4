...
{
    snprintf(out, out_len, "issp154-%02X%02X%02X", ext_addr[5], ext_addr[6], ext_addr[7]);
}
...
static bool find_device_id_by_host_suffix(uint32_t host_suffix, uint32_t *device_id)
{
    for (size_t i = 0; i < sizeof(s_devices) / sizeof(s_devices[0]); ++i) {
        ...
        uint32_t suffix = ((uint32_t)s_devices[i].ext_addr[5] << 16) |
                          ((uint32_t)s_devices[i].ext_addr[6] << 8) |
                          (uint32_t)s_devices[i].ext_addr[7];
        if (suffix == host_suffix) {
            *device_id = s_devices[i].device_id;
            return true;
        }
    }
    return false;
...
/// @brief Parse host-visible IDs generated by this central or hex/numeric IDs from the host.
static bool parse_device_id(const char *text, uint32_t *device_id)
{
    if (text == NULL || device_id == NULL) {
        return false;
    }

    const char *cursor = text;
    int base = 10;
    if (strncmp(cursor, "issp154-", 7) == 0) {
        cursor += 7;
        base = 16;
    } else if (strncmp(cursor, "0x", 2) == 0 || strncmp(cursor, "0X", 2) == 0) {
        base = 16;
    }
    ...
    if (strncmp(text, "issp154-", 7) == 0) {
        return find_device_id_by_host_suffix((uint32_t)parsed, device_id);
    }
...
/// @brief Return true when this device_id + seq was already processed, remembering the source address.
...
static const uint8_t *find_device_ext_addr(uint32_t device_id)
{
    for (size_t i = 0; i < sizeof(s_devices) / sizeof(s_devices[0]); ++i) {
        if (s_devices[i].valid && s_devices[i].device_id == device_id) {
            return s_devices[i].ext_addr;
        }
    }
    return NULL;
}
...
static bool send_radio_command(uint32_t device_id, uint8_t event_type, uint8_t value)
{
    ...
    const uint8_t *dst_ext_addr = find_device_ext_addr(device_id);
...
static void handle_host_line(char *line)
{
    ...
    if (parse_device_id(device_id_text, &device_id) &&
        command_to_event(capability_name, value, &event_type, &event_value)) {
        (void)send_radio_command(device_id, event_type, event_value);
    } else {
        ESP_LOGW(TAG,
